...
## 评审预算——每个 Plan 最多评审 2 次

每个 Plan（或每次变更）rook 最多评审 **2 次**：首次 + 至多 1 次复审，之后评审**永久关闭**。计数对象是评审次数，不是会话——用新开会话、新任务等方式对同一 Plan 取得第 3 次评审，同样违规。

1. **首次必须穷尽**：一份报告列全所有 finding（含边缘发现），扣着不报 = 服务缺陷。
2. **复审 = 验证修复 + 全量重扫**，报告即终局。复审仍 BLOCK：修完后**不再送审**，按 finding 报告处置。
3. **Owner 义务（Wopal）**：委派评审/复审时，prompt 必须写明预算（如「评审预算：本 Plan 最多 2 次评审——本轮列全所有 finding，无下一轮」）。
