...
PYTHON_COMMANDS="issue|plan|sync|archive|approve|submit|complete|verify|verify-switch|help|reset"
...
if [[ "$CMD" =~ ^($PYTHON_COMMANDS)$ ]]; then
    ...
    echo "Available commands: issue plan sync archive approve submit complete verify verify-switch help reset" >&2
    exit 1
fi
