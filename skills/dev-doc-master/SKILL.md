...
- **Phase `Advancing` is per phase, not a fixed set**: `Advancing` names the Capability Roadmap rows a phase moves and the step each reaches. An early phase may move every dimension; a focused phase may move one. A row the phase does not move is absent from `Advancing`, not listed as excluded.
