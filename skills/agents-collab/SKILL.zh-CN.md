...
- 小幅返工与复审重审：`reply` 原 task——快，保住上下文。
- 重型轮次先预估：`当前 + 本轮预估`——多 finding 修复批、大型 TDD 轮每轮耗 20–40%。
- 预估越过 ~50–60% → 新开 fae（带完整交接 prompt）或 Wopal 直接收尾；**绝不在历史任务中继续**。
- IDLE 时已超 ~50–60% → 会话已耗尽，不再喂下一轮。
- 以下情形同样新开：scope 实质变化；子 Agent 跑偏、循环、质量下滑。
- 不打断健康运行任务——仅受下方救援规则约束。
...
运行中 fae 超 ~60%、完成度不高、后续工作量大 → 输出质量快速下降，**必须**救援。用户亦可直接下令：

1. `wopal_task_abort`（纯停止），随即对 task 项目 `git status`。工作区状态即恢复点：干净，或带着本轮未提交变更。
2. 确认该状态前不碰任何东西。
3. `context_manage(action="compact", session_id="wopal-task-xxx")`；等 `[WOPAL TASK COMPACTED]`，期间不对该 task 做任何工具调用。
4. `wopal_task_reply` 发自包含恢复指令：worktree 路径、HEAD、剩余工作、探针结论、恢复点。压缩后会话只有摘要——默认它什么都不记得。
