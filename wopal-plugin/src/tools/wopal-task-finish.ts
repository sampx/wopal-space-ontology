...
export function createWopalTaskFinishTool(manager: SimpleTaskManager): ToolDefinition {
  return tool({
    description:
      "Task finisher. Terminates idle/completed task and deletes child session. Direct termination — child agent NOT woken up. Use after verification passed. Running tasks: interrupt via wopal_task_reply(interrupt=true) first.",
    args: {
