...
> 研究日期: 2026-03-15
> 研究对象: `labs/fork/sampx/oh-my-openagent`、`labs/ref-repos/opencode/packages/opencode`、`projects/ontology/agents/wopal/plugins`
...
| Wopal ontology | `projects/ontology` | 当前 Wopal 插件与部署体系 |
...
| 现有 Wopal 插件 | `projects/ontology/agents/wopal/plugins/wopal-plugin/src/runtime.ts` | Wopal 当前已实践的 hook 能力 |
...
`projects/ontology/agents/wopal/plugins/wopal-plugin/src/runtime.ts` 已证明 Wopal 侧具备以下现实基础：
...
不应把 orchestrator 能力继续叠加在 `wopal-plugin` 上，原因如下：

| 原因 | 说明 |
|------|------|
| 职责边界不同 | `wopal-plugin` 是规则层，orchestrator 是编排层 |
...
- `wopal-plugin` 已经证明 Wopal 能在 OpenCode 中稳定使用关键 transform hook
...
- 如果将 orchestrator 能力直接塞进 `wopal-plugin`，后续可维护性会明显下降
