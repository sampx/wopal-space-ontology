...
从远程拉取 main 和所有 type/* 分支的最新内容到本地仓库。

clone 模式：从 origin 拉取，fast-forward 到本地 main 和 type/* 分支。

fork 模式：先从 upstream 同步到副本镜像（origin），再从 origin 同步到本地。两层同步确保副本镜像和本地都拿到最新内容。

完成后，本地仓库拥有最新的 main 和 type/* 镜像。这一步操作本体仓库，发生在空间之外。

**第二步：space update（Space 级，应用到空间）**

从本地能力类型分支（type/*）同步到当前空间分支，把上游更新应用到用户实际工作的空间。

**两步之间的关系**：先更新本体仓库（拿到最新能力分支），再应用到空间。中间可以检查更新内容、验证兼容性。两条命令解耦，分别在不同上下文执行。

**上下层级间的维护**：main 的变更需要手动下行到 type/*。上游维护者负责把 main 的内容合并到各 type/* 分支（通过 `apply --from main --to type/<name>`），确保 type/* 始终是 main 的超集。fork 用户在自己仓库上贡献 main 层变更后，同样需要手动执行这步，然后 push 到副本。

**约束**：下行同步仅支持 fast-forward 合并。遇到分叉时报告状态，由 agent 引导用户手动处理。
...
fork 模式检查三层：
1. 上游 main/type/* vs 副本 main/type/* —— 副本镜像是否落后上游
2. 副本 main/type/* vs 本地 main/type/* —— 本地是否落后副本
3. 本地 type/* vs 空间分支 —— 空间分支是否落后能力类型分支

clone 模式检查两层：
1. 上游 main/type/* vs 本地 main/type/* —— 本地是否落后上游
2. 本地 type/* vs 空间分支 —— 空间分支是否落后能力类型分支

**新增能力分支检测**：除了已有分支的内容更新，还检查上游是否有本地尚未安装的新能力分支（如上游新增了 type/security）。检测到新分支时报告"新能力分支可用"，建议通过 ontology update 安装。

同层级比较（上游↔副本↔本地的同名分支）：比较分支的完整内容指纹。内容指纹相同则"已是最新"，不同则"有更新可用"。

跨层级比较（能力类型分支→空间分支）：空间分支是能力类型分支的超集，内容指纹必然不同。通过变更内容比对，识别"能力类型分支有但空间分支尚未包含的真正变更"，过滤掉已通过其他方式拥有的等效变更。

检测到更新时，展示文件层面的变更概要（哪些文件新增、修改、删除），配合上游最近的提交描述，让用户直观看到更新了什么。
 
**向上检查（有无待贡献或待同步的变更）**：

发现两类信号：

1. **空间分支上有未贡献的变更**：空间分支相对对应能力类型分支，有哪些变更还没有回流？通过变更内容比对识别真正独有的变更（过滤掉能力类型分支已有的等效变更）。帮助用户判断"我积累了哪些值得贡献的通用变更"。

2. **能力类型分支与副本/上游的一致性**：fork 模式下，本地能力类型分支相对副本是否有未推送的变更？副本相对上游是否有待同步的差异？帮助用户判断"我的贡献是否已经传播到位"。

**根据模式提示操作**：

检查结果结合用户的模式（clone/fork），给出操作建议：
- 向下有更新 → 建议 ontology update + space update
- 向上有未贡献变更（fork 模式）→ 提示考虑贡献
- 能力类型分支与上游不一致 → 提示同步方向
...
**约束**：下行同步仅 fast-forward；贡献仅 fork 模式；`apply` 自动检测删除文件并警告。

#### AI 辅助流程

Agent 帮助用户完成检查、更新和贡献的决策。核心模式：CLI 输出结构化状态，agent 解读后与用户讨论，再构造命令执行。决策由 agent 和用户共同完成。

**决策优先级**（从高到低）：

1. worktree 有未提交变更 → 先提交
2. 远程有未推送变更 → 先推送
3. 上游有内容更新 → 建议 ontology update
4. 副本有内容更新（fork 模式）→ 建议 ontology update（两层同步自动覆盖副本镜像）
5. 空间分支有少量定制变更 → 正常，可继续工作
6. 空间分支有大量未贡献变更 → 讨论是否贡献
...
**`/ontology-maintain` 命令**：Agent 使用此命令触发完整维护流程——收集状态（`ontology status` + `ontology list`）→ 分析建议（按决策框架排序）→ 报告确认（等待用户确认）→ 执行操作（每次操作后验证）。
