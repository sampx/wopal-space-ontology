...
| `update` | 拉取上游本体 type 更新到当前空间实例分支 | fork 模式两层同步（upstream → fork mirror → space）；clone 模式 fetch+merge；仅 fast-forward，diverged 报告 agent |
...
- `update` fork 模式两层同步：Layer 1 先同步 `upstream/<type-ref>` → `origin/<type-ref>`（fork 镜像），Layer 2 再同步到空间实例
...
- **Fork behind > 0**（fork 模式）→ fork 镜像落后上游，`wopal ontology update` 的两层同步会自动处理
...
| 4 | Fork behind > 0 | 建议 `wopal ontology update`（两层同步自动覆盖 fork 镜像） | `update` 先同步 fork 镜像再同步空间实例 |
