...
| Date | Type | Summary |
|---|---|---|
| 2026-06-13 | Updated | §6.3 Space Init — `--type` 能力选择、`space/<user>/<name>` 分支命名、user 自动解析、ontology registry 配置节点。 |
| 2026-06-13 | Updated | §6.8 Ontology Branch Model — `ontology list` 输出分层（能力分支 / 空间实例 / 开发分支）、远程追踪分支分类、`--type` 映射、`space status`/`ontology status` 共享 SpaceOntologyContext。 |
...
两者共享 `SpaceOntologyContext` resolver，输出当前空间的本体关联状态：

| 概念 | 值来源 | Git ref / path |
|---|---|---|
| Space | `space.name` | `space.path` |
| Ontology | `space.ontology` | `ontology.path` |
| Mode | `ontology.fork` / remotes | `origin` + `upstream` |
| User | `space.user` / branch / remote | `space.branch` |
| Type | `space.type` or `common` | `deriveTypeRef(space.type)` |

`ontology status` 使用 `deriveTypeRef(space.type)` 计算 ahead/behind，旧配置无 `type` 时 fallback 到 `main`。
