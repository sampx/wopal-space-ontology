...
第一层——内容指纹门控：比较分支的完整内容指纹（tree hash）。指纹相同则内容完全一致，标记"已是最新"并跳过后续分析。指纹不同则进入第二层。

第二层——变更内容比对：通过 patch-ID（`git cherry`）识别真正缺失的变更。合并采纳（squash merge）导致同一变更在不同分支上有不同 SHA——仅比较提交数量或指纹会产生虚假的"有更新"报告。patch-ID 基于变更内容计算，与 SHA 无关：`+` 标记真正缺失（需同步），`-` 标记等效已有（过滤掉，避免将旧更新合并到已有新变更上）。

分叉场景（双方各有独有变更，无法 fast-forward）时，双向 cherry 让 agent 一眼看出合并方向：每方的 `+` 就是从对方需要拿过来的变更，`-` 就是已等效不需要碰的。

检测到更新时，展示文件层面的变更概要（哪些文件新增、修改、删除），配合提交描述，让用户直观看到更新了什么。
