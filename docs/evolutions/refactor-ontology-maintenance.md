...
3. [x] 路由保留：`rg -c "ontology-evolution" skills/space-master/SKILL.md` ≥ 1（实测 3）；`SKILL.zh-CN.md` 同步含对应路由条目（实测 3，两镜像均 109 行、结构对齐）；两文件的本体段均无 `--include` / `--confirm` 用法示例（实测 0）。路径基准同 AC#1。
