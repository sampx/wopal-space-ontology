...
说明项目在父产品中的定位、承担的职责和明确边界。一句话核心职责 + 技术定位。

## 2. Capability Scope

列出项目拥有的目标态能力组。只描述边界：拥有的目标能力、明确排除的领域和委派边界。不含阶段时间、实现状态或交付进度。
...
- **Goal**: 本项目在本阶段的分工目标（一句话，≥20 字符，对齐父产品 phase doc 对本项目的期望）
...
- Goal 必须从父产品 phase doc 的 Involved Projects 推导本项目的分工目标
