...
命令文件存放于 `commands/`（共享）和 `commands/wopal/`（Wopal 专属）。模板见 `templates/command.md`。

**结构要求**：

1. **Frontmatter**：`description`（必填）、`subtask`（子任务命令必填）
2. **Input 声明**：紧跟标题，显式声明参数来源与含义
   - 无参数：`` **Input**: 无 ``
   - 单参数：`` **Input**: `$ARGUMENTS` (含义, required/optional) ``
   - 多参数：声明每个 `$1`、`2` 的含义，注意最大编号的 `$N` 具有 **rest 语义**（吃掉从第 N 个 token 起的所有剩余内容）
   - 子命令式：用 `| 用户输入 | 操作 |` 表格
3. **核心原则**（可选）：有执行约束时前置声明
4. **步骤**：Step 1→2→3→...，每步有明确输入、操作、输出
5. **输出格式**（可选）：提供示例
...
| 占位符 | 行为 |
|--------|------|
| `$ARGUMENTS` | 命令名后的完整原始字符串，不拆分 |
| `$1`、`$2`...`$N` | 按空格拆分的位置参数（支持引号分组），最大编号的 `$N` 吃掉剩余内容（rest 语义） |
| 无占位符 | 参数自动追加到模板末尾 |

替换顺序：先 `$1`...`$N` → 再 `$ARGUMENTS` → 无占位符则追加。
