...
describe("injectMemoryToMessage", () => {
  it("stores injectedRawText after successful injection", async () => {
    ...
    expect(sessionStore.get("ses_1")?.injectedRawText).toContain(
  ...
  it("skips injection when capabilities.memoryInjectionEnabled is false", async () => {
    const retrieveAndFormat = vi.fn().mockResolvedValue("# memory");
    const { ctx, sessionStore } = createMemoryCtx({
      retrieveAndFormat,
      capabilities: { memoryInjectionEnabled: false },
    });
...
// ---------------------------------------------------------------------------
// capabilities structure (memory injection gate lives in capabilities)
// ---------------------------------------------------------------------------

describe("injectMemoryToMessage capabilities", () => {
  it("skips injection when capabilities.memoryInjectionEnabled is false", async () => {
    const retrieveAndFormat = vi.fn().mockResolvedValue("# memory");
    const { ctx, sessionStore } = createMemoryCtx({
      retrieveAndFormat,
    ...
    sessionStore.upsert("ses_cap1", (state) => {
      state.lastUserPrompt = "hello";
      state.needsMemoryInjection = true;
    });
    ...
    await injectMemoryToMessage(ctx, "ses_cap1", messages, lastUserMsg);

    expect(lastUserMsg.parts!.length).toBe(1);
    expect(retrieveAndFormat).not.toHaveBeenCalled();
    // Flag not consumed when disabled (defense in depth)
    expect(sessionStore.get("ses_cap1")?.needsMemoryInjection).toBe(true);
  });

  it("defaults to enabled when capabilities field is omitted", async () => {
    const retrieveAndFormat = vi.fn().mockResolvedValue("# memory");
    const { ctx, sessionStore } = createMemoryCtx({ retrieveAndFormat });

    sessionStore.upsert("ses_cap2", (state) => {
      state.lastUserPrompt = "hello";
      state.needsMemoryInjection = true;
    });
    ...
    await injectMemoryToMessage(ctx, "ses_cap2", messages, lastUserMsg);

    expect(lastUserMsg.parts!.length).toBe(2);
    expect(sessionStore.get("ses_cap2")?.needsMemoryInjection).toBe(false);
  });

  it("injects when capabilities.memoryInjectionEnabled is true", async () => {
    const retrieveAndFormat = vi.fn().mockResolvedValue("# memory");
    const { ctx, sessionStore } = createMemoryCtx({
      retrieveAndFormat,
      capabilities: { memoryInjectionEnabled: true },
    });

    sessionStore.upsert("ses_cap3", (state) => {
      state.lastUserPrompt = "hello";
      state.needsMemoryInjection = true;
    });
    ...
    await injectMemoryToMessage(ctx, "ses_cap3", messages, lastUserMsg);
