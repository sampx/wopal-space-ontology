/**
 * Context Distillation Formatters
 *
 * Rendering for the session distillation preview/confirm reports.
 * Owned by the context module: distillation (preview → confirm) is an
 * LLM-driven context ability, so its user-facing guidance points at
 * `context_manage`, not `memory_manage`.
 */

import type { PreviewCandidate } from "./distill.js";
...
export const ECHO_REMINDER_DISTILL = [
  ...
  "distill 后必须等用户确认才能执行 context_manage action=confirm，严禁跳过展示直接 confirm。",
].join("\n");
...
export function formatPreviewReport(
  ...
  lines.push("- To write all candidates: `context_manage action=confirm`");
  lines.push(
