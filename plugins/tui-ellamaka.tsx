...
type Frame = { t: number; list: Ring[]; hold: Hold | undefined; release: Release | undefined; glow: Glow | undefined; spark: number }
...
function AnimatedLogo(props: { theme: ThemeLike; ink?: Color; idle?: boolean }) {
  const ctx = CTX
  ...
  const renderLine = (
    ...
    return Array.from(line).map((char, i) => {
      ...
      const pulseTop = state ? idle(off + i, y * 2, f, ctx, state) : { glow: 0, peak: 0, primary: 0 }
      const pulseBot = state ? idle(off + i, y * 2 + 1, f, ctx, state) : { glow: 0, peak: 0, primary: 0 }
      ...
      const inkTopTint = primaryMixTop > 0 ? tint(inkColor, theme.primary, primaryMixTop) : inkColor
      const inkBotTint = primaryMixBot > 0 ? tint(inkColor, theme.primary, primaryMixBot) : inkColor
      ...
      const inkPrimary = primaryMix > 0 ? tint(inkColor, theme.primary, primaryMix) : inkColor
...
const branding = (theme: ThemeLike): TuiSlotPlugin => ({
  slots: {
    ...
    session_prompt_right(ctx, value) {
      ...
      return (
        <text fg={s.textMuted}>
          <span style={{ fg: s.primary }}>ELLAMAKA</span>:{value.session_id.slice(0, 8)}
        </text>
      )
    },
  },
