...
**Local directory installs link by default (official CLI, verified on dsh 0.1.2-rc.1).** The official CLI `add <dir>` is a bare `pnpm add <dir>` under the hood, which always records a `link:` dependency — there is no copy option. Node ESM resolves a linked plugin's imports against the plugin's real path (the source directory), so the source directory must carry its own `node_modules`: run `npm install` in the plugin source directory after `add`. A registered-but-dependency-less link passes registration cleanly and then fails at boot with `Cannot find package '<peer>' imported from <source-dir>/lib/...` — registration success proves nothing about runnability. Registry-published plugins (e.g. dshmarket plugins) are unaffected: their dependencies download into the profile with the package. The ellamaka CLI copies instead of linking, so its installs do not need this step. If a future dsh version changes this behavior, re-verify with a clean-source-directory add + boot before deleting this note.
